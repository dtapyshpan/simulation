...
void SimulationWorker::setModelData( ModelData *const data )
{
  assert( data );
  curData = data;
...
void SimulationWorker::moveWater( ShuffleIndexes *const indexes, const GraphData &graph )
{
  ...
  for( int i = 0; i < height * width; ++i )
  {
    ...
    makePermutation( uk ); //random order of the neighbors
...
{
  cz.lock();
  ShuffleIndexes indexes( height, width );
  cz.unlock();
